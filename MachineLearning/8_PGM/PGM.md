...
> [!abstract] 概览
> 概率图模型 (PGM) 利用图结构表示高维随机变量之间的**条件独立性**关系，从而简化联合概率分布的计算。主要分为**有向图（贝叶斯网络）** 和**无向图（马尔可夫网络）** 两大类，核心工具包括因子分解、精确/近似推断以及参数/结构学习。
...
而本章所有的工作都是为了让这个联合概率的计算变得可行——通过引入**条件独立性假设**和**图结构**来简化高维联合分布。
...
$$
Chain\ Rule: p(x_1,x_2,\cdots,x_p)=p(x_1)p(x_2|x_1)p(x_3|x_2,x_1)=\prod\limits_{i=1}^pp(x_i|x_{i-1},...,x_1)
$$
...
1. **朴素贝叶斯**：在朴素贝叶斯中，作出了条件独立性假设，各维度相互独立的（太 naive 了）。
2. **Markov 性质**：在给定当前时刻的情况下，将来和过去是独立的 $x_j\perp x_{i+1}|x_i$（有时过去一个动作是影响未来好几步的，因此算不上完全独立）。
...
1. 有向图（一般为离散）：贝叶斯网络
2. 无向图（一般为离散）：马尔可夫网络
3. 高斯图（连续）：高斯贝叶斯和高斯马尔可夫网络

### 推断

1. 精确推断
2. 近似推断：确定性近似（如变分推断）和随机近似（如 MCMC）
...
上式说明了，A 发不发生是不影响的，A 与 C 是独立的，那么 $A\perp C|B$。正常情况下 A、C 不独立，B 被观测（图中去除掉）后 A、C 独立。
...
$$
p(A,B,C)=p(A)p(B|A)p(C|A)=p(A)p(B|A)p(C|A,B)
$$

这里左式是看图看出来的，右式是链式法则，那么就有 $p(C|A)=p(C|A,B)$。
上式说明了，B 发不发生是不影响的，因此有 $C\perp B|A$。
...
$$
\Longrightarrow p(C)=p(C|A) \Leftrightarrow C\perp A
$$

对这种结构默认情况下 A、C 就是独立的，$A,C$ 不与 $B$ 条件独立。

### D 划分

那么具体的，给定一个概率图，怎么划分条件独立性假设中的三个集合呢？或者怎么判断给出的三个集合是否满足？这里引入了 D 划分：

1. 对于类似上面图 1 和图 2 的关系，A 集合任一点到 B 集合任一点路径上的点都应在 $C$ 集合中。
2. 满足图 3 ，A 集合任一点到 B 集合任一点路径上的点都不能在 C 集合中。

如果给定一个网络，依照以上两个原则进行检查，发现不违反关系，那么给出的三个集合就是满足条件独立性的（也叫有**全局马尔可夫性**）。

> [!info] Markov 毯
> 在一个全局马尔可夫性的贝叶斯网络中，我们针对一个节点进行分析：
>
> $$
...
> $$
>
> 也就是说，对于节点 $i$，只有其**双亲、孩子、孩子的另一个双亲**这几部分相关，这些相关的部分又叫做 **Markov 毯**。
...
- **全局 Markov**：直接将条件独立的集合划分为 $x_A\perp x_B|x_C$ 三个集合。
- **局部 Markov**：对局部的节点，$x\perp (X-Neighbour(\mathcal{x}))|Neighbour(x)$。
- **成对 Markov**：对于成对的节点，$x_i\perp x_j|x_{-i-j}$，其中 $i,j$ 不能相邻。

> [!tip] 三种 Markov 性的等价性
> 事实上，全局、局部、成对三种 Markov 性是相互等价的。

有了这个条件独立性的划分，还需要因子分解来实际计算。引入**团**的概念：

> [!note] 团与最大团
> 图中节点的集合，集合中的节点之间相互都是连接的叫做**团**；如果不能再添加节点，那么叫**最大团**。
...
其中 $\phi(x_{ci})$ 叫做**势函数**，它必须是一个正值，可以记为：
...
$$
p(x)=\frac{1}{Z}\exp\left(-\sum\limits_{i=1}^KE(x_{ci})\right)
$$

这个分解和条件独立性等价（**Hammersley-Clifford 定理**），这个分布的形式也和指数族分布形式上相同，于是满足**最大熵原理**。
...
对于一个有向图，可以通过引入环的方式，将其转换为无向图（Tree-like graph），这个图就叫做道德图。但是上面的 BP 算法只对无环图有效，通过因子图可以变为无环图。
